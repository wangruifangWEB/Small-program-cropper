...
const TAIL_ICON = './icon/tailoring.png'; // 裁剪按钮
...
dragGraph.prototype = {
    ...
    paint() {
        ...
        if (this.selected) {
            ...
            if (this.type === 'text') {
                ...
                this.ctx.drawImage(TAIL_ICON, this.x + textWidth - 15, this.y - 15, 30, 30); //绘制裁剪
            } else {
                this.ctx.strokeRect(this.x, this.y, this.w, this.h);
                this.ctx.drawImage(DELETE_ICON, this.x - 15, this.y - 15, 30, 30);
                this.ctx.drawImage(DRAG_ICON, this.x + this.w - 15, this.y + this.h - 15, 30, 30);
                this.ctx.drawImage(TAIL_ICON, this.x + this.w - 15, this.y - 15, 30, 30); //绘制裁剪
            }
        }
        this.ctx.restore();
    },
    ...
    isInGraph(x, y) {
        ...
        // 旋转后的裁剪区域坐标
        const transformedTailCenter = this._rotatePoint(this.x + this.w, this.y, this.centerX, this.centerY, this.rotate);
        const transformTailX = transformedTailCenter[0] - tailW / 2;
        const transformTailY = transformedTailCenter[1] - tailH / 2;
        ...
        if (DEBUG_MODE) {
            ...
            this.ctx.strokeRect(transformTailX, transformTailY, tailW, tailH);
            // 标识移动区域
            this._drawBorder();
        }
        ...
        if (x - transformScaleX >= 0 && y - transformScaleY >= 0 &&
            ...
            // 删除区域
            return 'del';
        } else if (x - transformTailX >= 0 && y - transformTailY >= 0 &&
            transformTailX + tailW - x >= 0 && transformTailY + tailH - y >= 0) {
            // 裁剪区域
            return 'tail';
            console.log('裁剪区域=');
...
Component({
    ...
    methods: {
        ...
        start(e) {
            ...
            let lastTailIndex = null; // 记录裁剪的的索引
            this.drawArr && this.drawArr.forEach((item, index) => {
                ...
                if (action) {
                    ...
                    if (action === 'tail') {
                        let currentSrc; // 当前操作图片
                        if(typeof(this.drawArr[index].imgMess) != 'undefined'){
                            currentSrc = this.drawArr[index].imgMess.url;
                        }else{
                            currentSrc = this.drawArr[index].fileUrl;
                        }
                        console.log('currentSrc:' + currentSrc);
                        this.uploadTap(index, currentSrc);
                    }
                } else {
                    item.action = false;
                    item.selected = false;
        ...
        uploadTap(index, imgSrc) {
            ...
            // this.cropper.upload(options.imgSrc);//上传图片
            this.upload(imgSrc);//上传图片
        ...
        submit() {
            this.cropper.getImg((obj) => {
                console.log(obj);
                let _this = this;
                let ratio = obj.width / obj.height; //图片的真实宽高比例
                let imgWid = obj.width * 0.2, //设置图片显示宽度，
                imgHight = imgWid / ratio; //计算的高度值
                 
                if(typeof(_this.drawArr[_this.currentIndex].imgMess) == 'undefined'){
                    let imgMess={};//存储原图信息
                    imgMess['url']=this.drawArr[_this.currentIndex].fileUrl //存储之前操作的图片
                    imgMess['w']=this.drawArr[_this.currentIndex].w //存储之前操作的图片
                    imgMess['h']=this.drawArr[_this.currentIndex].h //存储之前操作的图片
                    this.drawArr[_this.currentIndex]['imgMess']=imgMess
                }

                // 截取新图信息更新
                this.drawArr[_this.currentIndex].fileUrl = obj.url
                this.drawArr[_this.currentIndex].w = imgWid
                this.drawArr[_this.currentIndex].h = imgHight
                
                this.setData({
                    originUrl: '',
                    cropperResult: obj.url
                })
                console.log(this.drawArr[_this.currentIndex]);
                this.drawArr[_this.currentIndex].selected = false;
                this.draw();
            });
        },
    }
